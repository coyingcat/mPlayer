...
func ReaderConverterCallback(_ converter: AudioConverterRef,
    ...
    data.withUnsafeBytes { (bytes: UnsafeRawBufferPointer) in
        let unsafeBufferPoint = bytes.bindMemory(to: UInt8.self)
        let unsafePoint = unsafeBufferPoint.baseAddress!
        memcpy((ioData.pointee.mBuffers.mData?.assumingMemoryBound(to: UInt8.self))!, unsafePoint, dataCount)
    }
